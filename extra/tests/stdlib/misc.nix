...
let
  inherit (lib)
    allUnique and attrNames attrsets attrsToList bitAnd bitOr bitXor
    boolToString callPackagesWith callPackageWith cartesianProduct cli
    composeExtensions composeManyExtensions concatLines concatMapAttrs
    concatMapStrings concatStrings concatStringsSep const escapeXML evalModules
    filter fix fold foldAttrs foldl foldl' foldlAttrs foldr functionArgs
    generators genList getExe getExe' groupBy groupBy' hasAttrByPath hasInfix id
    ifilter0 isStorePath lazyDerivation length lists listToAttrs makeExtensible
    makeIncludePath makeOverridable mapAttrs mapCartesianProduct matchAttrs
    mergeAttrs meta mod nameValuePair optionalDrvAttr optionAttrSetToDocList
    overrideExisting packagesFromDirectoryRecursive pipe range
    recursiveUpdateUntil removePrefix replicate runTests setFunctionArgs
    showAttrPath sort sortOn stringLength strings stringToCharacters systems
    tail take testAllTrue toBaseDigits toHexString toInt toIntBase10 toShellVars
    types updateManyAttrsByPath versions xor;
...
in runTests {
  ...
  */
  ...
  testXor = {
    expr =
      [ (xor true false) (xor true true) (xor false false) (xor false true) ];
  ...
  testIfilter0IndexOnly = {
    expr = length (ifilter0 (i: v: mod i 2 == 0) [
      (throw "0")
      (throw "1")
      (throw "2")
      (throw "3")
  ...
  testFoldl'StrictInitial = {
    expr =
      (builtins.tryEval (foldl' (acc: el: el) (throw "hello") [ ])).success;
    expected = false;
  };
  ...
  testListHasPrefixLazy = {
    expr =
      lists.hasPrefix [ 1 ] [ 1 (abort "lib.lists.hasPrefix is not lazy") ];
    expected = true;
  };
  ...
  testListCommonPrefixLong = let longList = genList (n: n) 100000;
  in {
  ...
  testSortOnIncomparable = {
    expr = map (x: x.f x.ok) (sortOn (x: x.ok) [
      {
        ok = 1;
        f = x: x;
      }
  ...
  testLongestValidPathPrefix_zero = {
    expr = attrsets.longestValidPathPrefix [ "a" (throw "do not use") ] {
      d = null;
    };
    expected = [ ];
  };
  ...
  testLongestValidPathPrefix_three_extra = {
    expr = attrsets.longestValidPathPrefix [ "a" "b" "c" ] {
      a.b.c.d = throw "nope";
    };
    expected = [ "a" "b" "c" ];
  };
  ...
  testFindFirstIndexExample1 = {
    expr = lists.findFirstIndex (x: x > 3)
      (abort "index found, so a default must not be evaluated") [ 1 6 4 ];
    expected = 1;
  };
  ...
  testFindFirstIndexEmpty = {
    expr = lists.findFirstIndex
      (abort "when the list is empty, the predicate is not needed") null [ ];
    expected = null;
  };
  ...
  testFindFirstIndexNone = {
    expr = builtins.tryEval (lists.findFirstIndex (x: x == 2) null [
      1
  ...
  testMergeAttrsListExampleMany = let
    list = genList (n:
      listToAttrs (genList (m:
        ...
        in nameValuePair str str) 100)) 100;
  in {
  ...
  in {
    expr = {
      isReverseToListToAttrs = builtins.listToAttrs (attrsToList exampleAttrs)
        == exampleAttrs;
      isReverseToAttrsToList =
        attrsToList (builtins.listToAttrs exampleSingletonList)
        == exampleSingletonList;
  ...
  testAttrsToListsCanDealWithFunctions =
    testingEval (attrsToList { someFunc = a: a + 1; });
  ...
  testGetExe'FailureFirstArg =
    testingThrow (getExe' "not a derivation" "executable");

  testGetExe'FailureSecondArg =
    testingThrow (getExe' { type = "derivation"; } "dir/executable");
