...
pkgs.runCommand "stdlib-tests-nix-${nix.version}" {
  ...
  nativeBuildInputs = [ nix pkgs.gitMinimal ]
  ...
  cp -r ${libPath} lib
  echo "Running lib/tests/stdlib/modules.sh"
  bash lib/tests/stdlib/modules.sh
  ...
  echo "Running lib/tests/stdlib/filesystem.sh"
  TEST_LIB=$PWD/lib bash lib/tests/stdlib/filesystem.sh

  echo "Running lib/tests/stdlib/sources.sh"
  TEST_LIB=$PWD/lib bash lib/tests/stdlib/sources.sh

  echo "Running lib/tests/stdlib/fileset.sh"
  TEST_LIB=$PWD/lib bash lib/tests/stdlib/fileset.sh
