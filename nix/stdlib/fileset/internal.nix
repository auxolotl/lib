...
let
  inherit (builtins)
    isAttrs isPath isString nixVersion pathExists readDir split trace typeOf
    fetchGit;

  inherit (lib.attrsets)
    attrNames attrValues mapAttrs mapAttrsToList optionalAttrs zipAttrsWith;

  inherit (lib.filesystem) pathType;

  inherit (lib.lists)
    all commonPrefix concatLists elemAt filter findFirst findFirstIndex foldl'
    head length sublist tail;

  inherit (lib.path) append splitRoot hasStorePathPrefix splitStorePath;

  inherit (lib.path.subpath) components join;

  inherit (lib.strings)
    isStringLike concatStringsSep substring stringLength hasSuffix
    versionAtLeast;
...
in rec {
  ...
  migrations = [
    ...
    (filesetV0:
      let parts = splitRoot filesetV0._internalBase;
      in filesetV0 // {
        _internalVersion = 1;
      ...
      if value._internalVersion > _currentVersion then
        throw ''
          ${context} is a file set created from a future version of the file set library with a different internal representation:
              - Internal version of the file set: ${
                toString value._internalVersion
              }
        ...
        let
          ...
          migrationsToApply = sublist value._internalVersion
            (_currentVersion - value._internalVersion) migrations;
        in foldl' (value: migration: migration value) value migrationsToApply
      else
        value
  ...
  _coerceMany = functionContext: list:
    let
      filesets = map
        ({ context, value, }: _coerce "${functionContext}: ${context}" value)
        list;

      # Find the first value with a base, there may be none!
      firstWithBase =
        findFirst (fileset: !fileset._internalIsEmptyWithoutBase) null filesets;
      ...
      differentIndex = findFirstIndex (fileset:
        ...
        !fileset._internalIsEmptyWithoutBase && firstBaseRoot
        != fileset._internalBaseRoot) null filesets;
      # Only evaluates `differentIndex` if there are any elements with a base
    in if firstWithBase != null && differentIndex != null then
      throw ''
        ${functionContext}: Filesystem roots are not the same:
            ${(head list).context}: Filesystem root is "${
              toString firstBaseRoot
            }"
  ...
  _normaliseTreeFilter = path: tree:
    if tree == "directory" || isAttrs tree then
      let
        ...
        normalisedSubtrees =
          mapAttrs (name: _normaliseTreeFilter (path + "/${name}")) entries;
      ...
      in if all isNull subtreeValues then
        null
  ...
  */
  _normaliseTreeMinimal = path: tree:
    if tree == "directory" || isAttrs tree then
      let
        entries = _directoryEntries path tree;
        normalisedSubtrees =
          mapAttrs (name: _normaliseTreeMinimal (path + "/${name}")) entries;
        subtreeValues = attrValues normalisedSubtrees;
      ...
      in if all (value: value == "emptyDir") subtreeValues then
        "emptyDir"
      ...
      || all (value: isString value) subtreeValues then
        "directory"
      ...
      else if all (value: isNull value || value == "emptyDir")
      subtreeValues then
        null
    ...
    let
      ...
      traceTreeAttrs = prevLine: indent: tree:
        foldl' (prevLine: name:
          ...
          in if subtree == null || subtree == "emptyDir" then
          ...
          # Do print this node but no further recursion needed
            thisLine) prevLine (attrNames tree);

      # Evaluating this will print the first line
      firstLine = if tree == null || tree == "emptyDir" then
        trace "(empty)" null
      else
        trace "${toString base}${treeSuffix tree}" null;
    in if isAttrs tree then traceTreeAttrs firstLine "" tree else firstLine;
  ...
  _toSourceFilter = fileset:
    let
      ...
      baseString = if fileset._internalBaseComponents == [ ] then
    ...
    in if fileset._internalIsEmptyWithoutBase || tree == null then
      empty
    else
      nonEmpty;
  ...
  _fromSourceFilter = root: sourceFilter:
    let
      ...
      rootString = "/"
        + concatStringsSep "/" (components (splitRoot root).subpath);
    in if rootPathType == "directory" then
  ...
  _toList = fileset:
    let
      recurse = path: tree:
        if isAttrs tree then
          concatLists
          (mapAttrsToList (name: value: recurse (path + "/${name}") value) tree)
        else if tree == "directory" then
          ...
          [ path ];
    in if fileset._internalIsEmptyWithoutBase then
      [ ]
    else
  ...
  _shortenTreeBase = targetBaseComponents: fileset:
    ...
    in recurse (length targetBaseComponents);
    ...
    let
      recurse = index: tree:
        ...
        # Recurse with the tree under the right component (which might not exist)
          recurse (index + 1)
          (tree.${elemAt targetBaseComponents index} or null)
        else
        ...
        #   because we don't need to recurse any more since `index == length longestBaseComponents`
          tree;
    in recurse (length fileset._internalBaseComponents) fileset._internalTree;
  ...
  _unionMany = filesets:
    let
      ...
      filesetsWithBase =
        filter (fileset: !fileset._internalIsEmptyWithoutBase) filesets;
  ...
  _intersection = fileset1: fileset2:
    let
      ...
      commonBaseComponentsLength =
        ...
        length (commonPrefix fileset1._internalBaseComponents
      ...
      longestBaseFileset = if commonBaseComponentsLength
      == length fileset1._internalBaseComponents then
      # The common prefix is the same as the first path, so the second path is equal or longer
        fileset2
      else if commonBaseComponentsLength
      == length fileset2._internalBaseComponents then
      ...
      tree1 =
        _lengthenTreeBase longestBaseFileset._internalBaseComponents fileset1;
      tree2 =
        _lengthenTreeBase longestBaseFileset._internalBaseComponents fileset2;

      # With two filesetTree's with the same base, we can compute their intersection
      resultTree = _intersectTree tree1 tree2;
    in if resultIsEmptyWithoutBase then
      _emptyWithoutBase
    else
  ...
  _intersectTree = lhs: rhs:
    ...
    # Both sides are attribute sets, we can recurse for the attributes existing on both sides
      mapAttrs (name: _intersectTree lhs.${name})
  ...
  _difference = positive: negative:
    let
      ...
      commonBaseComponentsLength =
        # TODO: Have a `lib.lists.commonPrefixLength` function such that we don't need the list allocation from commonPrefix here
        length (commonPrefix positive._internalBaseComponents
      ...
      negativeTreeWithPositiveBase = if commonBaseComponentsLength
      == length positive._internalBaseComponents then
      ...
      == length negative._internalBaseComponents then
    ...
    in if positive._internalIsEmptyWithoutBase then
      _emptyWithoutBase
      ...
      fromDir = path:
        mapAttrs (name: type:
          if type == "directory" then
            ...
            fromFile name type) (readDir path);
      ...
      recurse = focusedStorePath:
        mapAttrs (name: type:
          if type == "directory" then
            ...
            type) (builtins.readDir focusedStorePath);
    in _create localPath (recurse storePath);
  ...
  _fromFetchGit = function: argument: path: extraFetchGitAttrs:
    let
      ...
      tryStorePath = if pathExists (path + "/.git") then
      ...
      # - Importing the entire path could include Git-tracked files
        throw ''
          lib.fileset.${function}: The ${argument} (${
            toString path
      ...
      in if !pathExists (path + "/.git") then
        throw "lib.fileset.${function}: Expected the ${argument} (${
          toString path
        }) to point to a local working tree of a Git repository, but it's not."
      else
        _mirrorStorePath path fetchResult.outPath;
    in if !isPath path then
      throw
      "lib.fileset.${function}: Expected the ${argument} to be a path, but it's a ${
        typeOf path
      } instead."
