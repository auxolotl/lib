...
let
  inherit (import ./internal.nix { inherit lib; })
    _coerce _singleton _coerceMany _toSourceFilter _fromSourceFilter _toList
    _unionMany _fileFilter _printFileset _intersection _difference _fromFetchGit
    _fetchGitSubmodulesMinver _emptyWithoutBase;

  inherit (builtins) isBool isList isPath pathExists seq typeOf nixVersion;
...
in {
  ...
  maybeMissing = path:
    if !isPath path then
      if isStringLike path then
        ...
        throw "lib.fileset.maybeMissing: Argument is of type ${
          typeOf path
  ...
  */
  trace =
    ...
    */
    ...
    in seq (_printFileset actualFileset) (x: x);
  ...
  */
  traceVal =
    ...
    */
    ...
    in seq (_printFileset actualFileset)
    ...
    */
    ...
    in if !isPath root then
      if root ? _isLibCleanSourceWith then
        ...
        throw "lib.fileset.toSource: `root` is of type ${
          typeOf root
        }, but it should be a path instead."
        # Currently all Nix paths have the same filesystem root, but this could change in the future.
        # See also ../path/README.md
    else if !fileset._internalIsEmptyWithoutBase && rootFilesystemRoot
    != filesetFilesystemRoot then
      throw ''
      ...
      throw "lib.fileset.toSource: `root` (${
        toString root
    ...
    else if !fileset._internalIsEmptyWithoutBase
    && !hasPrefix root fileset._internalBase then
      throw ''
  ...
  */
  unions =
    ...
    if !isList filesets then
      throw "lib.fileset.unions: Argument is of type ${
        typeOf filesets
  ...
  */
  intersection =
    ...
    in _intersection (elemAt filesets 0) (elemAt filesets 1);
  ...
  */
  difference =
    ...
    in _difference (elemAt filesets 0) (elemAt filesets 1);
  ...
  fileFilter =
    ...
    */
    ...
    if !isFunction predicate then
      throw "lib.fileset.fileFilter: First argument is of type ${
        typeOf predicate
      ...
      if path._type or "" == "fileset" then
        ...
        throw "lib.fileset.fileFilter: Second argument is of type ${
          typeOf path
        }, but it should be a path instead."
    else if !pathExists path then
      throw "lib.fileset.fileFilter: Second argument (${
        toString path
      }) is a path that does not exist."
    else
  ...
  */
  fromSource = source:
    ...
    in if !isPath path then
      ...
      throw "lib.fileset.fromSource: The source origin (${
        toString path
  ...
  */
  gitTracked =
    ...
    */
    path:
    _fromFetchGit "gitTracked" "argument" path { };
  ...
  */
  gitTrackedWith = {
    ...
    */
    ...
    && versionOlder nixVersion _fetchGitSubmodulesMinver then
      throw
