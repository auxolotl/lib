    ...
    */
    ...
    let
      ...
      directoryEntryToAttrPair = subdirectory: basename: type:
        let path = subdirectory + "/${basename}";
        in if type == "regular" then {
          ...
          throw ''
            lib.filesystem.packagesFromDirectoryRecursive: Unsupported file type ${type} at path ${
      ...
      packagesFromDirectory = path:
        let defaultPackagePath = path + "/package.nix";
        in if pathExists defaultPackagePath then
          callPackage defaultPackagePath { }
        else
          mapAttrs' (directoryEntryToAttrPair path)
          (packageDirectoryEntries path);
    in packagesFromDirectory directory;
}
