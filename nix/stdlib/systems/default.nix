...
let
  inherit (lib)
    any filterAttrs foldl hasInfix isFunction isList isString mapAttrs optional
    optionalAttrs optionalString removeSuffix replaceStrings toUpper;
    ...
    let
      ...
      final = {
        ...
        libDir = if final.isLinux then
          if final.isx86_64 || final.isMips64 || final.isPower64 then
            "lib64"
          else
            "lib"
        else
        ...
        ubootArch = if final.isx86_32 then
          "x86" # not i386
        else if final.isMips64 then
        ...
        selectEmulator = pkgs:
          ...
          in if pkgs.stdenv.hostPlatform.canExecute final then
            ''${pkgs.runtimeShell} -c '"$@"' --''
          else if final.isWindows then
            "${wine}/bin/wine${
              optionalString (final.parsed.cpu.bits == 64) "64"
            }"
        ...
        architectures.predicates // args // {
          rust = rust // {
            ...
            platform = rust.platform or { } // {
              ...
              target-family = if args ? rust.platform.target-family then
                args.rust.platform.target-family
              else if args ? rustc.platform.target-family then
                (
                  # Since https://github.com/rust-lang/rust/pull/84072
                  # `target-family` is a list instead of single value.
                  let f = args.rustc.platform.target-family;
                  in if isList f then f else [ f ])
              else
                optional final.isUnix "unix"
                ++ optional final.isWindows "windows";

              # https://doc.rust-lang.org/reference/conditional-compilation.html#target_vendor
              vendor = let inherit (final.parsed) vendor;
            ...
            rustcTarget = let
              ...
              # TODO: deprecate args.rustc in favour of args.rust after 23.05 is EOL.
            in args.rust.rustcTarget or args.rustc.config or "${cpu_}-${vendor_}-${kernel.name}${
              optionalString (abi.name != "unknown") "-${abi.name}"
            }";

            # The name of the rust target if it is standard, or the json file
            # containing the custom target spec.
            rustcTargetSpec = rust.rustcTargetSpec or (if rust ? platform then
              builtins.toFile (final.rust.rustcTarget + ".json")
              (toJSON rust.platform)
            else
            ...
            cargoEnvVarTarget = replaceStrings [ "-" ] [ "_" ]
...
in {
  inherit architectures doubles elaborate equals examples flakeExposed inspect
    parse platforms;
}
