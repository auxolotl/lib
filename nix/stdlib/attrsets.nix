...
let
  ...
  inherit (lib.lists)
    foldr foldl' concatMap elemAt all partition groupBy take foldl;
in rec {
    ...
    let
      ...
      attrByPath' = n: s:
        (if n == lenAttrPath then
          ...
          (let attr = elemAt attrPath n;
          in if s ? ${attr} then attrByPath' (n + 1) s.${attr} else default));
    in attrByPath' 0 set;
  ...
  */
  hasAttrByPath = attrPath: e:
    let
      lenAttrPath = length attrPath;
      hasAttrByPath' = n: s:
        (n == lenAttrPath || (let attr = elemAt attrPath n;
        in if s ? ${attr} then hasAttrByPath' (n + 1) s.${attr} else false));
    in hasAttrByPath' 0 e;
  ...
  /* *
     ...
     **Laws**:
     ...
     :::
  */
  longestValidPathPrefix = attrPath: v:
    let
      lenAttrPath = length attrPath;
      getPrefixForSetAtIndex =
        ...
        # so we return the whole path.
          ...
          let attr = elemAt attrPath remainingPathIndex;
          in if remainingSet ? ${attr} then
            getPrefixForSetAtIndex
  ...
  */
  matchAttrs = pattern: attrs:
    ...
    all ( # Compare equality between `pattern` & `attrs`.
      attr:
      ...
      in if isAttrs lhs then isAttrs rhs && matchAttrs lhs rhs else lhs == rhs))
    (attrNames pattern);
  ...
  */
  ...
  # DEPRECATED
  zipWithNames =
    warn "lib.zipWithNames is a deprecated alias of lib.zipAttrsWithNames."
    zipAttrsWithNames;
