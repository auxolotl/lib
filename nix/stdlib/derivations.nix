...
let
  ...
  showMaybeAttrPosPre = prefix: attrName: v:
    ...
    in if pos == null then
      ...
      "${prefix}${pos.file}:${toString pos.line}:${toString pos.column}";

  showMaybePackagePosPre = prefix: pkg:
    if pkg ? meta.position && isString pkg.meta.position then
      "${prefix}${pkg.meta.position}"
    else
      "";
in {
  ...
  lazyDerivation = args@{
    ...
    let
      ...
      checked = throwIfNot (derivation.type or null == "derivation")
        "lazyDerivation: input must be a derivation." throwIfNot
        ...
        (derivation.outputs == outputs) ''
          ...
          Actual outputs of the derivation${
            showMaybePackagePosPre ''
              ...
              ${
                lib.generators.toPretty { multiline = false; }
                derivation.outputs
              }
    ...
    } // genAttrs outputs (outputName: checked.${outputName}) // passthru;
