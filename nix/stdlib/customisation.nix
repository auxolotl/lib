...
let
  ...
  inherit (lib)
    ...
    filterAttrs optionalString flip pathIsDirectory head pipe isDerivation
    listToAttrs mapAttrs seq flatten deepSeq warnIf isInOldestRelease extends;
...
in rec {
    ...
    in mirrorArgs (origArgs:
      let
        ...
        overrideWith = newArgs:
          origArgs
          // (if isFunction newArgs then newArgs origArgs else newArgs);

        # Re-call the function but with different arguments
        overrideArgs =
          mirrorArgs (newArgs: makeOverridable f (overrideWith newArgs));
        # Change the result of the function call by applying g to it
        overrideResult = g:
          makeOverridable (mirrorArgs (args: g (f args))) origArgs;
      in if isAttrs result then
        result // {
    ...
    let
      ...
      prettySuggestions = suggestions:
        if suggestions == [ ] then
          ...
          ", did you mean ${concatStringsSep ", " (lib.init suggestions)} or ${
  ...
  callPackagesWith = autoArgs: fn: args:
    let
      ...
      mkAttrOverridable = name: _:
        makeOverridable (mirrorArgs (newArgs: (f newArgs).${name})) origArgs;
    in if isDerivation pkgs then
  ...
  */
  hydraJob = drv:
    ...
    in if drv == null then null else deepSeq drv' drv';
  ...
  */
  ...
  /* *
     ...
     ```
  */
  makeScopeWithSplicing' = { splicePackages, newScope, }:
    { otherSplices,
