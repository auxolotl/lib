...
{ lib }:
let
  inherit (builtins) isString isPath split match typeOf storeDir;

  inherit (lib.lists)
    length head last genList elemAt all concatMap foldl' take drop;
...
in {
  ...
  */
  hasPrefix = path1:
    ...
    let path1Deconstructed = deconstructPath path1;
    in path2:
  ...
  */
  removePrefix = path1:
    ...
    let
      ...
      success = take path1Length path2Deconstructed.components
        == path1Deconstructed.components;
  ...
  */
  splitRoot =
    ...
    path:
    assert assertMsg (isPath path) "lib.path.splitRoot: Argument is of type ${
        typeOf path
      }, but a path was expected";
  ...
  */
  hasStorePathPrefix = path:
    let deconstructed = deconstructPath path;
    in assert assertMsg (isPath path)
