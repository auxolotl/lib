...
let
  ...
  inherit (lib) boolToString filter isString readFile;
...
in {
  ...
  inherit pathIsGitRepo commitIdFromGitRepo cleanSource cleanSourceWith
    cleanSourceFilter pathHasContext canCleanSource sourceByRegex
    sourceFilesBySuffices trace;
}
